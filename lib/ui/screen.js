...
function createScreen(state) {
  ...
  if (global.__jestronaut_ui__) {
    const { screen, widgets, startTicker, state } = global.__jestronaut_ui__;
    screen.realloc();
    return {
      screen, widgets,
      renderAll: () => renderAll(screen, widgets, state),
      refreshOpenSuiteDetail: () => refreshOpenSuiteDetail(widgets, state),
      startTicker,
    };
  }
