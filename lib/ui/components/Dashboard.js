...
import { TestDetailOverlay } from './TestDetailOverlay.js';
...
export function Dashboard({ store }) {
  ...
  // Always block Jest's stdin listener — TUI handles all keypresses
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow || input === 'j') {
      if (s.testDetailOpen) {
        s.testDetailScrollOffset = (s.testDetailScrollOffset || 0) + 1;
      } else if (s.suiteDetailOpen) {
        moveSuiteDetailCursor(s, 1);
      } else if (s.focus === 'results') {
        if (!s.resultItems.length) return;
